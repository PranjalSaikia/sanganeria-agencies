...
import TempInput from './Edit/TempInput';
import TempView from './comp/TempView';
import TempTable from './Edit/TempTable';
import InvoiceDet from './Edit/InvoiceDet';
...
class EditSales extends Component {
    ...
    finalSubmit(data) {
        ...
        if(data.type === data.original_type){
            this.finalSubmitConfirm(data);
        }else{
            confirmAlert({
                title: 'Confirm to submit',
                message: 'The invoice will be migrated',
                buttons: [
                    {
                        label: 'Yes',
                        onClick: this.finalSubmitConfirm.bind(this,data)
                    },
                    {
                        label: 'No'
                    }
    ...
    finalSubmitConfirm = (data) => {
        //console.log(data);
       PostData('/api/update_invoice.php', data)
            .then((resp) => {
                //console.log(resp)
                if (resp.status === '200') {
